"""
重排序器模块
实现重排序策略：Cross-Encoder、MMR、集成重排序等
"""
import asyncio
...
class CrossEncoderReranker(BaseReranker):
    ...
    async def rerank(
        ...
        try:
            ...
            logger.info(
                f"Cross-Encoder重排序完成，耗时: {timer.elapsed:.3f}秒，处理 {len(documents)} 个文档")
    ...
    async def rerank(
        ...
        try:
            with Timer() as timer:
                ...
                # 选择第一个文档（相关性最高的）
                first_idx = max(
                    remaining, key=lambda i: documents[i].get('score', 0))
            ...
            logger.info(
                f"MMR重排序完成，耗时: {timer.elapsed:.3f}秒，返回 {len(reranked_docs)} 个文档")
...
class EnsembleReranker(BaseReranker):
    ...
    async def rerank(
        ...
        try:
            with Timer() as timer:
                ...
                for i, result in enumerate(results):
                    ...
                    for rank, doc in enumerate(result):
                        doc_id = doc.get(
                            'content_hash', str(doc.get('id', rank)))
            ...
            logger.info(
                f"集成重排序完成，耗时: {timer.elapsed:.3f}秒，返回 {len(final_results)} 个文档")
